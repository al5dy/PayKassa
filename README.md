...
Each URL pair defaults independently to `home_url('/')`. **External PayKassa callback base URL (optional)** controls only Invoice Payment Notification and Cryptocurrency Transaction Processor URLs. **External PayKassa browser return base URL (optional)** controls only successful payment and malfunction returns; set it to the public origin where customers actually perform checkout so the browser sends that origin's login and WooCommerce session cookies. Both overrides use the same strict validation and require HTTPS in Live mode. They do not alter WordPress, WooCommerce, order, REST or hosted-payment URLs.
